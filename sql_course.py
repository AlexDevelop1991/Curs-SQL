...
# TIMESTAMP WITH TIMEZONE -  содержит информацию о годе,месяце,дне,часе,минуте,секунде,долях секунды + часовой пояс.
# TIMESTAMP WITH  LOCAL TIMEZONE -  содержит информацию о годе,месяце,дне,часе,минуте,секунде,долях секунды +  локальный часовой пояс.
# INTERVAL YEAR TO MONTH - интервал который содержит значения год и месяц
# INTERVAL DATE TO SECOND - это интервал времени который исчесляется днями,часами,минутами и секундами то есть обозначает период времени.
# CLOB - в этом типе данных можно хранить огромные текстовые элементы
# BLOB - бинарные огромные объекты (фото,видео)
# LONG - устаревший тип,предпочтительней использовать CLOB.
# ROWID - это зашифрованный тип он относится к ORACLE 
